...
public class FiveOutOfNinetyDrawFactory {
    ...
    FiveOutOfNinetyDrawFactory(Logger logger) {
        this.logger = logger;
    }
            ...
            return Draw.newDraw()
                    ...
                    .hits(List.of(
                            Hit.newHit()
                                    ...
                                    .prize(Long.parseLong((values[4].substring(0, values[4].length() - 3)).replaceAll(SPACE, EMPTY)))
                                    .currency(Currency.getInstance(CURRENCY_HUF))
                                    .build(),
                            Hit.newHit()
                                    .hits(4)
                                    .tickets(Integer.parseInt(values[5]))
                                    .prize(Long.parseLong((values[6].substring(0, values[6].length() - 3)).replaceAll(SPACE, EMPTY)))
                                    .currency(Currency.getInstance(CURRENCY_HUF))
                                    .build(),
                            Hit.newHit()
                                    .hits(3)
                                    .tickets(Integer.parseInt(values[7]))
                                    .prize(Long.parseLong((values[8].substring(0, values[8].length() - 3)).replaceAll(SPACE, EMPTY)))
                                    .currency(Currency.getInstance(CURRENCY_HUF))
                                    .build(),
                            Hit.newHit()
                                    .hits(2)
                                    .tickets(Integer.parseInt(values[9]))
                                    .prize(Long.parseLong((values[10].substring(0, values[10].length() - 3)).replaceAll(SPACE, EMPTY)))
                                    .currency(Currency.getInstance(CURRENCY_HUF))
                                    .build()))
                    .drawnNumbers(List.of(DrawnNumbers.newDrawnNumbers().drawType(DrawType.MANUAL).numbers(List.of(
                            ...
                            Integer.parseInt(values[12].trim()),
                            Integer.parseInt(values[13].trim()),
                            Integer.parseInt(values[14].trim()),
                            Integer.parseInt(values[15].trim()))).build()))
                    .build();
        } catch (Exception exception) {
            logger.error("The CSV line cannot be processed: {}", csvLine);
            throw exception;
        }
    }
}
